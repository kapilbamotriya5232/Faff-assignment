...
export default function HomePage() {
  ...
  const handleSearchResults = useCallback(
    (results: TaskType[], query: string) => {
      setSearchResults(results)
      setActiveSearchQuery(query) // Keep track of the query that produced these results
      setIsSearching(true)
    },
    [],
  )
  ...
  const taskListFilters = {
    ...
    assignedToId:
      assignedToMeFilter && currentUser ? currentUser.id : undefined,
  }
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4 items-end'>
            ...
            <div className='min-w-0 flex items-center pt-4 md:pt-0 justify-self-start md:justify-self-auto'>
              <label className='flex items-center space-x-2 cursor-pointer group'>
                ...
                />
                <span className='text-sm font-medium text-slate-700 group-hover:text-blue-600 transition duration-150'>
                  Assigned to me
                </span>
              </label>
            </div>
